...
exports.Stream = internals.Stream = function (needle) {
    ...
    this._haystack = new Vise(null, needle.lastPos);
...
internals.Stream.prototype._write = function (chunk, encoding, next) {
    ...
    if (this._haystack.length) {
        ...
        if (notChecked) {
            while (notChecked < this._haystack.length &&
                !this._haystack.startsWith(this._needle.value, notChecked, this._haystack.length - notChecked)) {

                ++notChecked;
            }
        }

        this._flush(this._haystack.length - notChecked);
    }

...
internals.Stream.prototype._write = function (chunk, encoding, next) {
    ...
    if (match === -1 &&
        ...
        this._flush(this._haystack.length - chunk.length, emits);
    }

    while (match !== -1) {
        this._flush(match, emits);
        this._haystack.shift(this._needle.length);
        emits.push({ type: 'needle' });
    ...
    // Delay emitting until next tick

    process.nextTick(function () {

        for (var i = 0, il = emits.length; i < il; ++i) {
            var emit = emits[i];
            self.emit(emit.type, emit.data);
        }
...
internals.Stream.prototype._flush = function (pos, emits) {

    var chunks = this._haystack.shift(pos);
    for (var i = 0, il = chunks.length; i < il; ++i) {
        emits.push({ type: 'haystack', data: chunks[i] });
    }
};
